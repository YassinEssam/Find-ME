...
#     #return render(request, 'lost/lost.html', {'Lostperson':Lostperson.objects.all()})#.filter(age__exact=20), 'aml': Lostperson.objects.get(name='aml')}) # this path start from template
#     if request.method == 'POST':
#         first_name = request.POST.get('first_name')
#         last_name = request.POST.get('last_name')
#         name = first_name # + last_name
...
#         data = Lostperson(name=name, phone_number=phone_number, address=address, age=age, finder_name=finder_name, city=city, date=date, photo=photo)
#         data.save()
